...
sub_totals AS (
  SELECT
    ...
    SUM(CASE WHEN UPPER(TRIM(s.sub_certification_type)) IN ('MBE', 'WBE', 'MWBE', 'SEDBE', 'CBE')
    ...
    SUM(CASE WHEN s.sub_certification_type IS NULL 
              OR UPPER(TRIM(s.sub_certification_type)) NOT IN ('MBE', 'WBE', 'MWBE', 'SEDBE', 'CBE')
             THEN COALESCE(s.sub_amount, 0) 
...
SELECT 
  ...
  -- Sub details (keep all cert types, but only qualifying ones count in certified_spend)
