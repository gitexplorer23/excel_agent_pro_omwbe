...
def _canon(v):
    ...
    # treat all null-like values the same
    try:
        import pandas as pd
        if v is None or (isinstance(v, float) and pd.isna(v)) or (not isinstance(v, float) and pd.isna(v)):
            return ""
    except Exception:
        if v is None:
            return ""
    ...
    # trimmed string for everything else
    return str(v).strip()
